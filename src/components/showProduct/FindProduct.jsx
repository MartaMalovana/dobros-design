...
export default function FindProduct ({newData}) {
    ...
    const find = (e) => {
        ...
        const products = data.filter(el => el.number === e.currentTarget.number.value);
        ...
        <SearchContainer>
            <Form onSubmit={find}>
                <Number>
                    ...
                    <Input id='number' required placeholder={t('search.example')} pattern='[0-9]{0,6}-{0,1}\d*' title='Вводьте, будь-ласка, лише цифри' onChange={() => setNotFound(false)}></Input>
                    <Underline />
                </Number>
                <LabelSelect htmlFor='category'>{t('search.section')}</LabelSelect>
                <Select id='category'>
                    <Option value="all">{t('search.category.all')}</Option>
                    {regions.map(el => <Option value={el.name} key={el.name}>{i18next.language === 'ua' ? el.content : el.contentEng}</Option>)}
                    <Option value="kozhuhy">{t('search.category.kozhuhy')}</Option>
                    <Option value="golovni-ubory">{t('search.category.golovniUbory')}</Option>
